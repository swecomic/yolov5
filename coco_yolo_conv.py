...
def check_dir(path):
    if path == 'colab':
        ...
        images_dir_path = 'D:/dataset/xray/Train/Data'
        json_file_path = 'D:/dataset/xray/Train/Meta/CoCo/coco_rapiscan.json'
...
def data_view(images):
    df = pd.DataFrame(images)
    df['type'] = df['path'].str.split('\\').str[8]
    df['class'] = df['path'].str.split('\\').str[7]

    result = df.pivot_table(index='class', columns='type', values='id', aggfunc='count')
...
if __name__ == '__main__':
    """parameters"""
    images_dir_path, json_file_path = check_dir('local_jy')
    ...
    """image count"""
    images_nb = data_view(images)
    images_nb.to_clipboard(sep=',')
